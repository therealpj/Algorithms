/******************************************************************************
 *  A custom variant of ALgorithms 4e's TrieST program, which implements a trie
 *  symbol table for the 26 alphabets.
 *  Dependencies: StdIn.javap
 *
 ******************************************************************************/
...
public class Custom<Value> {
	...
	for (int i = 0; i < prefix.length(); i++) {
	    c = prefix.charAt(i);
	    if (x.next[c - 65] == null) return false;
	    x = x.next[c - 65];
	}
     ...
     * @throws IllegalArgumentException if {@code key} is {@code null}
     */
    public boolean containsR(String key) {
	...
	if (x == null || x.val == 0 || x.val == -1) return false;
	return true;
    }
	...
	if (x == null) return;
	if (x.val != -1) results.enqueue(prefix.toString());
	for (char c = 0; c < R; c++) {
	    prefix.append(c);
